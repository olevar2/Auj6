"""
MetaTrader 5 Tick Data Provider (DEPRECATED - USE MetaAPI)

This provider has been deprecated in favor of MetaAPI for cross-platform compatibility.
Use MetaApiProvider instead for tick data access.

Migration Path:
- Replace MT5TickDataProvider with MetaApiProvider
...
from ..core.exceptions import DataProviderError, DeprecationWarning
from ..core.logging_setup import get_logger

logger = get_logger(__name__)


class MT5TickDataProvider(BaseDataProvider):
    """
    DEPRECATED: MetaTrader 5 tick data provider.
    ...
    def __init__(self, config_manager, config: Optional[Dict[str, Any]] = None):
        ...
        logger.warning("MT5TickDataProvider is DEPRECATED. Use MetaApiProvider instead.")
        
        # Basic initialization to maintain compatibility
        self.available_symbols = []
        super().__init__(
            name="mt5_tick_deprecated",
    ...
    async def connect(self) -> bool:
        ...
        logger.warning("MT5TickDataProvider connection attempted - provider is deprecated")
        return False
    ...
    async def get_tick_data(self, symbol: str, start_time: Optional[datetime] = None,
        ...
        logger.error("MT5TickDataProvider.get_tick_data called - provider is deprecated. Use MetaApiProvider.")
        raise DataProviderError("MT5TickDataProvider is deprecated. Use MetaApiProvider for tick data.")
        
    async def get_ohlcv_data(self, symbol: str, timeframe: Timeframe,
                           start_time: Optional[datetime] = None,
                           end_time: Optional[datetime] = None, count: Optional[int] = None) -> Optional[pd.DataFrame]:
        """Return None as provider is deprecated."""
        logger.error("MT5TickDataProvider.get_ohlcv_data called - provider is deprecated. Use MetaApiProvider.")
        raise DataProviderError("MT5TickDataProvider is deprecated. Use MetaApiProvider for OHLCV data.")
