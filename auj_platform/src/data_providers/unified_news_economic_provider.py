...
class UnifiedNewsEconomicProvider(BaseDataProvider):
    ...
    async def get_news_data(self, 
        ...
        try:
            ...
            if self.primary_source == 'forexfactory':
                news_events = await self._get_forexfactory_news(symbols, start_time, end_time, count)
            elif self.primary_source == 'investing':
                news_events = await self._get_investing_news(symbols, start_time, end_time, count)
            ...
            logger.info(f"Retrieved {len(news_events)} news events")
    ...
    async def _get_forexfactory_news(self, symbols: Optional[List[str]], 
                                   start_time: datetime, end_time: datetime, 
                                   count: int) -> List[NewsEvent]:
        """Get news from ForexFactory."""
        try:
            news_url = f"{self.sources['forexfactory']['base_url']}/news"
            news_events = []
            
            async with self.session.get(news_url) as response:
                if response.status != 200:
                    logger.error(f"ForexFactory news request failed: {response.status}")
                    return []
                ...
                # Look for news items
                news_container = soup.find('div', class_='content') or soup.find('main') or soup.find('body')
                if not news_container:
                    logger.warning("Could not find news container on ForexFactory")
                    return []
                
                # Find news links and headlines
                news_links = news_container.find_all('a', href=True)
                processed_count = 0
                ...
                for link in news_links:
                    ...
                    if not text or len(text) < 10:
                        continue
                    if any(skip in href.lower() for skip in ['calendar', 'login', 'register', 'contact', 'privacy']):
                        continue
                    
                    # Extract timestamp
                    timestamp = self._extract_timestamp_from_context(link)
                    ...
                    news_event = NewsEvent(
                        ...
                        affected_symbols=self._filter_symbols_by_content(text, symbols),
                        sentiment=self._analyze_sentiment(text)
                    )
                    
                    # Filter by symbols if specified
                    if symbols and not news_event.affected_symbols:
                        continue
            ...
            logger.error(f"Error getting ForexFactory news: {e}")
            return []
    
    async def _get_investing_news(self, symbols: Optional[List[str]], 
                                start_time: datetime, end_time: datetime, 
                                count: int) -> List[NewsEvent]:
        """Get news from Investing.com."""
        try:
            # Similar implementation to ForexFactory
            # This would be implemented based on Investing.com's structure
            logger.info("Investing.com news retrieval - placeholder implementation")
            return []
    ...
    async def get_economic_calendar(self, 
                                  ...
                                  categories: Optional[List[str]] = None) -> Optional[EconomicCalendar]:
        """
        Get economic calendar data.
        
        Args:
            start_time: Start date for calendar
            end_time: End date for calendar
            currencies: Currency codes to filter
            impact_levels: Impact levels to filter
            categories: Event categories to filter
