"""
Evening Star Indicator - Advanced Three-Candle Bearish Reversal Pattern Detection
==============================================================================

This indicator implements sophisticated evening star pattern detection with advanced
...
class EveningStarIndicator(StandardIndicatorInterface):
    """
    Advanced Evening Star Pattern Indicator
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        super().__init__(name="EveningStarIndicator", parameters=default_params)
        ...
        logging.info(f"EveningStarIndicator initialized with parameters: {self.parameters}")
    ...
    def calculate_raw(self, data: pd.DataFrame) -> Dict[str, Any]:
        """Calculate evening star patterns with advanced analysis"""
        try:
            if len(data) < 50:
            ...
            detected_patterns = self._detect_evening_star_patterns(enhanced_data)
            ...
            raise IndicatorCalculationException(
                self.name, "calculation", f"Evening star calculation failed: {str(e)}", e
            )
    ...
    def _detect_evening_star_patterns(self, data: pd.DataFrame) -> List[EveningStarPattern]:
        """Detect evening star patterns with sophisticated analysis"""
        patterns = []
        ...
        for i in range(self.parameters['trend_lookback'], len(data) - 1):
            ...
            candle1 = data.iloc[i-2]  # First candle (bullish)
            candle2 = data.iloc[i-1]  # Star candle
            candle3 = data.iloc[i]    # Third candle (bearish)
            
            # Check for evening star pattern
            if not self._is_evening_star_pattern(candle1, candle2, candle3):
                continue
            
            # Calculate pattern components
            gap_up_quality = self._calculate_gap_quality(candle1, candle2, 'up')
            gap_down_quality = self._calculate_gap_quality(candle2, candle3, 'down')
            ...
            # Calculate bearish confirmation
            bearish_confirmation = self._calculate_bearish_confirmation(candle3)
            
            # Calculate pattern strength
            pattern_strength = self._calculate_pattern_strength(
                gap_up_quality, gap_down_quality, star_doji_quality,
                bearish_confirmation, trend_context_score
            )
            
            if pattern_strength >= 0.7:  # High threshold for quality patterns
                pattern = EveningStarPattern(
    ...
    def _is_evening_star_pattern(self, candle1: pd.Series, candle2: pd.Series, 
                               candle3: pd.Series) -> bool:
        """Check if three candles form an evening star pattern"""
        # 1. First candle should be bullish (large body)
        if candle1['close'] <= candle1['open']:
            return False
        if candle1['body_ratio'] < 0.6:
            return False
        
        # 2. Second candle should gap up and be small (star characteristics)
        if candle2['low'] <= candle1['high']:  # Should gap up
            return False
        if candle2['body_ratio'] > self.parameters['min_star_body_ratio']:
            return False
        
        # 3. Third candle should be bearish and gap down or penetrate first candle
    ...
    def _assess_trend_context(self, data: pd.DataFrame, pattern_end_index: int) -> float:
        """Assess the uptrend context before the pattern"""
        ...
        if price_trend > 0.1:  # Strong uptrend
            context_factors.append(1.0 * 0.3)
        elif price_trend > 0.05:  # Moderate uptrend
            context_factors.append(0.8 * 0.3)
        elif price_trend > 0:  # Weak uptrend
        ...
        # 3. RSI overbought condition (20% weight)
        rsi_factor = 0
        if latest['rsi'] > 80:
            rsi_factor = 1.0
        elif latest['rsi'] > 70:
        ...
        # 4. Bollinger Band position (15% weight)
        bb_factor = min(latest['bb_position'], 1.0) if not pd.isna(latest['bb_position']) else 0.5
    ...
    def _calculate_bearish_confirmation(self, candle: pd.Series) -> float:
        """Calculate bearish confirmation strength of the third candle"""
        ...
        close_position = (candle['close'] - candle['low']) / candle['total_range'] if candle['total_range'] > 0 else 0
        close_strength = 1.0 - close_position  # Lower close is better for bearish
        confirmation_factors.append(close_strength * 0.3)
        
        # 3. Lower shadow size (20% weight)
        lower_shadow_strength = 1.0 - candle['lower_shadow_ratio']  # Prefer small lower shadow
        confirmation_factors.append(lower_shadow_strength * 0.2)
    ...
    def _calculate_pattern_strength(self, gap_up_quality: float, gap_down_quality: float,
                                  star_doji_quality: float, bearish_confirmation: float,
        ...
        strength_components = [
            ...
            bearish_confirmation * 0.2,
            trend_context_score * 0.15
        ]
        
        return sum(strength_components)
    
    def _analyze_volume_confirmation(self, patterns: List[EveningStarPattern], 
                                   data: pd.DataFrame) -> List[EveningStarPattern]:
    ...
    def _analyze_support_resistance(self, patterns: List[EveningStarPattern], 
                                  data: pd.DataFrame) -> List[EveningStarPattern]:
    ...
    def _calculate_support_resistance_factor(self, data: pd.DataFrame, pattern_idx: int) -> float:
        ...
        try:
            ...
            pattern_high = data.iloc[pattern_idx - 2]['high']  # First candle high
            
            sr_factors = []
            
            # 1. Historical resistance at pattern level (40% weight)
            resistance_touches = 0
            price_tolerance = pattern_high * 0.005  # 0.5% tolerance
            
            for i in range(len(context_data) - 3):
                if abs(context_data.iloc[i]['high'] - pattern_high) <= price_tolerance:
                    resistance_touches += 1
            
            resistance_strength = min(resistance_touches / 3, 1.0)
            sr_factors.append(resistance_strength * 0.4)
            
            # 2. Pivot point analysis (30% weight)
            pivot_highs = context_data[context_data['pivot_high']]['high']
            if len(pivot_highs) > 0:
                nearest_resistance = min(abs(pivot_highs - pattern_high))
                pivot_factor = 1.0 / (1.0 + nearest_resistance / pattern_high * 100)
                sr_factors.append(pivot_factor * 0.3)
            else:
                sr_factors.append(0.15)
            
            # 3. Volume at resistance level (30% weight)
            resistance_volumes = []
            ...
            if resistance_volumes:
                ...
                volume_factor = avg_resistance_volume / avg_volume if avg_volume > 0 else 1.0
    ...
    def _validate_institutional_activity(self, patterns: List[EveningStarPattern], 
                                       data: pd.DataFrame) -> List[EveningStarPattern]:
    ...
    def _calculate_institutional_validation_score(self, data: pd.DataFrame, pattern_idx: int) -> float:
        ...
        try:
            ...
            # For evening star, expect negative institutional flow
            flow_factor = 0
            if obv_trend < 0:
                flow_factor += 0.5
            if ad_trend < 0:
    ...
    def _predict_reversal_probability(self, patterns: List[EveningStarPattern], 
                                    data: pd.DataFrame) -> List[EveningStarPattern]:
    ...
    def _extract_reversal_features(self, data: pd.DataFrame, pattern_idx: int, 
                                 pattern: EveningStarPattern) -> List[float]:
        ...
        try:
            ...
            features = [
                ...
                context_data['macd_hist'].iloc[-1] if not pd.isna(context_data['macd_hist'].iloc[-1]) else 0,
    ...
    def _train_reversal_model(self, patterns: List[EveningStarPattern], features: List[List[float]]):
    ...
    def _heuristic_reversal_probability(self, pattern: EveningStarPattern) -> float:
    ...
    def _generate_pattern_analytics(self, patterns: List[EveningStarPattern]) -> Dict[str, Any]:
        ...
        return {
            ...
            'strong_resistance_patterns': len([p for p in recent_patterns if p.support_resistance_factor > 0.7])
        }
    ...
    def _analyze_current_trend_context(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'is_uptrend': current['close'] > current['sma_20'],
            'ma_alignment': {
                'short_above_medium': current['sma_5'] > current['sma_10'],
                'medium_above_long': current['sma_10'] > current['sma_20'],
                'price_above_ma': current['close'] > current['sma_5']
            },
    ...
    def _generate_reversal_signals(self, patterns: List[EveningStarPattern], 
    ...
    def _assess_pattern_reliability(self, patterns: List[EveningStarPattern]) -> Dict[str, Any]:
    ...
    def _assess_market_conditions(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'trend_regime': 'uptrend' if current['trend_strength'] > 0.6 else 'sideways',
            'momentum_state': 'overbought' if current['rsi'] > 70 else 'neutral' if current['rsi'] > 30 else 'oversold'
        }
    
    def _generate_signal(self, value: Dict[str, Any], data: pd.DataFrame) -> Tuple[Optional[SignalType], float]:
        """Generate trading signal based on evening star analysis"""
        ...
        # Strong signal when we have high-quality pattern in uptrend with good confirmations
