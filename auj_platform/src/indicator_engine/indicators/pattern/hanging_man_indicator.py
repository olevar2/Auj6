"""
Hanging Man Indicator - Advanced Single-Candle Bearish Reversal Pattern Detection
================================================================================

This indicator implements sophisticated hanging man pattern detection with advanced
...
class HangingManPattern:
    """Represents a detected hanging man pattern"""
...
class HangingManIndicator(StandardIndicatorInterface):
    """
    Advanced Hanging Man Pattern Indicator
    
    Features:
    - Precise hanging man identification with body-to-shadow ratio analysis
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        super().__init__(name="HangingManIndicator", parameters=default_params)
        ...
        logging.info(f"HangingManIndicator initialized with parameters: {self.parameters}")
    ...
    def calculate_raw(self, data: pd.DataFrame) -> Dict[str, Any]:
        """Calculate hanging man patterns with advanced analysis"""
        try:
            if len(data) < 40:
            ...
            detected_patterns = self._detect_hanging_man_patterns(enhanced_data)
            ...
            raise IndicatorCalculationException(
                self.name, "calculation", f"Hanging man calculation failed: {str(e)}", e
            )
    ...
    def _detect_hanging_man_patterns(self, data: pd.DataFrame) -> List[HangingManPattern]:
        """Detect hanging man patterns with sophisticated analysis"""
        patterns = []
        ...
        for i in range(self.parameters['trend_lookback'], len(data)):
            ...
            if not self._is_hanging_man_pattern(candle):
                continue
    ...
    def _is_hanging_man_pattern(self, candle: pd.Series) -> bool:
        """Check if candle meets hanging man criteria"""
        # Hanging man has same structure as hammer but appears in uptrend
        return (candle['lower_shadow_ratio'] >= self.parameters['min_lower_shadow_ratio'] and
                candle['body_ratio'] <= self.parameters['max_body_ratio'] and
                candle['upper_shadow_ratio'] <= self.parameters['max_upper_shadow_ratio'] and
                candle['body_position'] >= 0.7 and
                candle['volatility_ratio'] >= 0.8)
    
    def _assess_uptrend_context(self, data: pd.DataFrame, candle_index: int) -> float:
        """Assess uptrend context before hanging man"""
    ...
    def _calculate_pattern_strength(self, candle: pd.Series, trend_context_score: float) -> float:
        """Calculate overall hanging man pattern strength"""
        strength_components = [
            min(candle['lower_shadow_ratio'] / 0.8, 1.0) * 0.3,  # Lower shadow quality
            (1.0 - candle['body_ratio'] / self.parameters['max_body_ratio']) * 0.25,  # Body size quality
            (1.0 - candle['upper_shadow_ratio'] / self.parameters['max_upper_shadow_ratio']) * 0.2,  # Upper shadow quality
            min(max((candle['body_position'] - 0.7) / 0.3, 0), 1) * 0.15,  # Body position quality
            trend_context_score * 0.1  # Trend context
        ]
        
        return sum(strength_components)
    
    def _analyze_volume_confirmation(self, patterns: List[HangingManPattern], 
                                   data: pd.DataFrame) -> List[HangingManPattern]:
    ...
    def _calculate_volume_confirmation_score(self, data: pd.DataFrame, pattern_idx: int) -> float:
        ...
        try:
            ...
            # Distribution volume (high volume with limited price progress suggests distribution)
            price_progress = abs(candle['close'] - candle['open']) / candle['total_range'] if candle['total_range'] > 0 else 0
            distribution_factor = volume_surge * (1.0 - price_progress)
            
            return (surge_score * 0.4 + volume_trend_score * 0.3 + 
                   min(distribution_factor / 2, 1.0) * 0.3)
            
        except Exception:
            return 0.5
    
    def _analyze_resistance_strength(self, patterns: List[HangingManPattern], 
                                   data: pd.DataFrame) -> List[HangingManPattern]:
    ...
    def _calculate_resistance_strength_score(self, data: pd.DataFrame, pattern_idx: int) -> float:
        ...
        try:
            ...
            hanging_man_high = data.iloc[pattern_idx]['high']
            
            # Historical resistance touches
            resistance_touches = 0
            price_tolerance = hanging_man_high * 0.01
            
            for i in range(len(context_data) - 1):
                if abs(context_data.iloc[i]['high'] - hanging_man_high) <= price_tolerance:
            ...
            if len(pivot_highs) > 0:
                nearest_pivot_distance = min(abs(pivot_highs - hanging_man_high))
                pivot_confluence = 1.0 / (1.0 + nearest_pivot_distance / hanging_man_high * 100) * 0.3
            ...
            volume_resistance_factor = 0.1
            if resistance_volumes:
                avg_resistance_volume = np.mean(resistance_volumes)
                avg_volume = context_data['volume'].mean()
                volume_resistance_factor = min(avg_resistance_volume / avg_volume, 1.0) * 0.2 if avg_volume > 0 else 0.1
            
            return resistance_strength * 0.4 + pivot_confluence + volume_resistance_factor + 0.1
            
        except Exception:
            return 0.5
    
    def _analyze_distribution_signals(self, patterns: List[HangingManPattern], 
                                    data: pd.DataFrame) -> List[HangingManPattern]:
    ...
    def _calculate_distribution_signal_score(self, data: pd.DataFrame, pattern_idx: int) -> float:
        ...
        try:
            ...
            # High volume with small price movement suggests distribution
            distribution_ratio = (volume_action / avg_volume) / (price_action + 0.001) if avg_volume > 0 else 1.0
            distribution_score = min(distribution_ratio / 5, 1.0)
            
            return flow_score * 0.4 + cmf_score * 0.3 + distribution_score * 0.3
            
        except Exception:
            return 0.5
    
    def _analyze_pattern_purity(self, patterns: List[HangingManPattern], 
                              data: pd.DataFrame) -> List[HangingManPattern]:
    ...
    def _calculate_pattern_purity_score(self, data: pd.DataFrame, pattern_idx: int) -> float:
        ...
        try:
            hanging_man_candle = data.iloc[pattern_idx]
            ...
            if surrounding_shadows:
                hanging_man_shadow = hanging_man_candle['lower_shadow_ratio']
                max_surrounding = max(surrounding_shadows)
                isolation_quality = max(0, min(1, (hanging_man_shadow - max_surrounding) / hanging_man_shadow)) * 0.3
            
            # Geometric perfection
            ideal_proportions = {'lower_shadow': 0.75, 'body': 0.15, 'upper_shadow': 0.1}
            deviations = [
                abs(hanging_man_candle['lower_shadow_ratio'] - ideal_proportions['lower_shadow']),
                abs(hanging_man_candle['body_ratio'] - ideal_proportions['body']),
                abs(hanging_man_candle['upper_shadow_ratio'] - ideal_proportions['upper_shadow'])
            ]
            
            geometric_score = max(0, 1.0 - sum(deviations) / 0.5) * 0.25
            
            # Color and context consistency
            is_bearish = hanging_man_candle['close'] < hanging_man_candle['open']
            prev_trend = data.iloc[max(0, pattern_idx - 5):pattern_idx]['close'].mean()
            current_close = hanging_man_candle['close']
            
            color_score = 0.3 if (current_close > prev_trend and is_bearish) else 0.2
            
            # Shadow symmetry
            shadow_ratio = hanging_man_candle['upper_shadow_ratio'] / hanging_man_candle['lower_shadow_ratio'] if hanging_man_candle['lower_shadow_ratio'] > 0 else 1
    ...
    def _predict_reversal_probability(self, patterns: List[HangingManPattern], 
                                    data: pd.DataFrame) -> List[HangingManPattern]:
    ...
    def _extract_reversal_features(self, data: pd.DataFrame, pattern_idx: int, 
                                 pattern: HangingManPattern) -> List[float]:
    ...
    def _train_reversal_model(self, patterns: List[HangingManPattern], features: List[List[float]]):
    ...
    def _heuristic_reversal_probability(self, pattern: HangingManPattern) -> float:
    ...
    def _generate_pattern_analytics(self, patterns: List[HangingManPattern]) -> Dict[str, Any]:
    ...
    def _generate_reversal_signals(self, patterns: List[HangingManPattern], 
    ...
    def _analyze_market_structure(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            'resistance_structure': {
                ...
                'current_position': 'near_resistance' if current['bb_position'] > 0.7 else 'neutral'
            },
            'volume_structure': {
            ...
            'momentum_structure': {
                ...
                'trend_exhaustion': current['rsi'] > 75 if not pd.isna(current['rsi']) else False
            }
        }
    ...
    def _assess_pattern_reliability(self, patterns: List[HangingManPattern]) -> Dict[str, Any]:
    ...
    def _generate_signal(self, value: Dict[str, Any], data: pd.DataFrame) -> Tuple[Optional[SignalType], float]:
        """Generate trading signal based on hanging man analysis"""
