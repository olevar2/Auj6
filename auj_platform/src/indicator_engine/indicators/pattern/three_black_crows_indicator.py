"""
Three Black Crows Indicator - Advanced Three-Candle Bearish Reversal Pattern Detection
====================================================================================

This indicator implements sophisticated three black crows pattern detection, identifying
powerful bearish reversal signals through consecutive declining bearish candles.
"""
...
class ThreeBlackCrowsPattern:
    """Represents a detected three black crows pattern"""
    ...
    gap_analysis: float
    price_descent_rate: float
    volume_confirmation: float
    pattern_purity: float
    exhaustion_signals: float


class ThreeBlackCrowsIndicator(StandardIndicatorInterface):
    """
    Advanced Three Black Crows Pattern Indicator
    
    Features:
    - Precise three-candle bearish sequence detection
    - Advanced body consistency and progression analysis
    - Comprehensive volume trend and acceleration tracking
    - ML-enhanced reversal potential prediction
    - Gap analysis and price descent rate calculation
    - Shadow analysis for pattern purity validation
    - Momentum acceleration and exhaustion detection
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        default_params = {
            ...
            'max_upper_shadow_ratio': 0.25,      # Maximum upper shadow for clean bearish candles
            'min_decline_progression': 0.6,      # Minimum price decline consistency
            ...
            'ml_prediction': True,
            'gap_analysis': True,
            'exhaustion_detection': True
        }
        
        if parameters:
            default_params.update(parameters)
            
        super().__init__(name="ThreeBlackCrowsIndicator", parameters=default_params)
        ...
        logging.info(f"ThreeBlackCrowsIndicator initialized with parameters: {self.parameters}")
    ...
    def calculate_raw(self, data: pd.DataFrame) -> Dict[str, Any]:
        """Calculate three black crows patterns with advanced analysis"""
        try:
            if len(data) < 60:
            ...
            detected_patterns = self._detect_three_black_crows_patterns(enhanced_data)
            ...
            if self.parameters['exhaustion_detection']:
                detected_patterns = self._analyze_exhaustion_signals(detected_patterns, enhanced_data)
            
            if self.parameters['ml_prediction'] and detected_patterns:
                detected_patterns = self._predict_reversal_potential(detected_patterns, enhanced_data)
            ...
            return {
                ...
                'descent_statistics': self._calculate_descent_statistics(detected_patterns)
            }
            
        except Exception as e:
            raise IndicatorCalculationException(
                self.name, "calculation", f"Three black crows calculation failed: {str(e)}", e
            )
    ...
    def _detect_three_black_crows_patterns(self, data: pd.DataFrame) -> List[ThreeBlackCrowsPattern]:
        """Detect three black crows patterns with sophisticated analysis"""
        patterns = []
        ...
        for i in range(self.parameters['trend_context_periods'], len(data) - 2):
            ...
            # Check basic three black crows criteria
            if self._is_three_black_crows_sequence(candle1, candle2, candle3):
                pattern = self._create_three_black_crows_pattern(
    ...
    def _is_three_black_crows_sequence(self, c1: pd.Series, c2: pd.Series, c3: pd.Series) -> bool:
        ...
        # Limited upper shadows for clean bearish candles
        if not (c1['upper_shadow_ratio'] <= self.parameters['max_upper_shadow_ratio'] and
                c2['upper_shadow_ratio'] <= self.parameters['max_upper_shadow_ratio'] and
                c3['upper_shadow_ratio'] <= self.parameters['max_upper_shadow_ratio']):
            return False
        
        # Progressive decline in closing prices
    ...
    def _create_three_black_crows_pattern(self, data: pd.DataFrame, candle_idx: int,
                                        c1: pd.Series, c2: pd.Series, c3: pd.Series) -> Optional[ThreeBlackCrowsPattern]:
        """Create three black crows pattern with comprehensive analysis"""
        try:
            ...
            price_descent_rate = self._calculate_price_descent_rate(c1, c2, c3)
            
            # Trend context analysis
            trend_context_score = self._assess_three_crows_trend_context(data, candle_idx)
            
            # Momentum analysis
            momentum_acceleration = self._analyze_momentum_acceleration(data, candle_idx)
            
            # Calculate pattern strength
            pattern_strength = self._calculate_three_crows_strength(
                candle_progression, body_consistency, shadow_analysis,
                trend_context_score, momentum_acceleration, gap_analysis
            )
            
            pattern = ThreeBlackCrowsPattern(
                timestamp=c3.name,
                ...
                price_descent_rate=price_descent_rate,
    ...
    def _analyze_candle_progression(self, c1: pd.Series, c2: pd.Series, c3: pd.Series) -> float:
        ...
        elif c2['open'] < c1['close']:  # Gap down is acceptable
            progression_factors.append(0.15)
        
        if c3_open_in_c2_body:
            progression_factors.append(0.25)
        elif c3['open'] < c2['close']:  # Gap down is acceptable
            progression_factors.append(0.15)
        
        # Declining close sequence strength
        decline1 = (c1['close'] - c2['close']) / c1['close']
        decline2 = (c2['close'] - c3['close']) / c2['close']
        
        avg_decline = (decline1 + decline2) / 2
        decline_score = min(avg_decline / 0.03, 1.0)  # 3% average decline = max score
        progression_factors.append(decline_score * 0.3)
        
        # Consistent decline pattern
        if decline1 > 0 and decline2 > 0:
            consistency = 1.0 - abs(decline1 - decline2) / max(decline1, decline2)
    ...
    def _analyze_shadow_characteristics(self, c1: pd.Series, c2: pd.Series, c3: pd.Series) -> float:
        ...
        # Upper shadows should be minimal (clean bearish candles)
        upper_shadow_scores = []
        for candle in [c1, c2, c3]:
            shadow_score = max(0, 1.0 - candle['upper_shadow_ratio'] / self.parameters['max_upper_shadow_ratio'])
            upper_shadow_scores.append(shadow_score)
        
        avg_upper_shadow_score = np.mean(upper_shadow_scores)
        shadow_factors.append(avg_upper_shadow_score * 0.4)
        
        # Lower shadows analysis
        lower_shadow_ratios = [c1['lower_shadow_ratio'], c2['lower_shadow_ratio'], c3['lower_shadow_ratio']]
        
        # Moderate lower shadows are acceptable (showing some support testing)
        lower_shadow_scores = []
        for ratio in lower_shadow_ratios:
            if ratio <= 0.15:  # Ideal range
                lower_shadow_scores.append(1.0)
            elif ratio <= 0.3:  # Acceptable range
                lower_shadow_scores.append(0.7)
            else:
                lower_shadow_scores.append(max(0, 1.0 - (ratio - 0.3) / 0.2))
        
        avg_lower_shadow_score = np.mean(lower_shadow_scores)
        shadow_factors.append(avg_lower_shadow_score * 0.35)
        
        # Shadow progression (declining lower shadows show increasing weakness)
        if c3['lower_shadow_ratio'] <= c2['lower_shadow_ratio'] <= c1['lower_shadow_ratio']:
            shadow_factors.append(0.25)
        elif c3['lower_shadow_ratio'] <= c2['lower_shadow_ratio'] or c2['lower_shadow_ratio'] <= c1['lower_shadow_ratio']:
    ...
    def _analyze_gap_sequence(self, c1: pd.Series, c2: pd.Series, c3: pd.Series) -> float:
        ...
        # Calculate gaps
        gap1 = (c1['close'] - c2['open']) / c1['close']
        gap2 = (c2['close'] - c3['open']) / c2['close']
        
        # Small gaps down are positive for the pattern
        for gap in [gap1, gap2]:
            if 0 <= gap <= 0.005:  # Small gap down
                gap_factors.append(0.25)
            elif 0 <= gap <= 0.015:  # Moderate gap down
                gap_factors.append(0.15)
            elif gap < 0:  # Gap up (negative for pattern)
                gap_factors.append(max(0, 0.1 + gap / 0.01))  # Penalty for gap up
            else:  # Large gap down
        ...
        # Progressive gapping down
        if gap1 > 0 and gap2 > gap1:
            gap_factors.append(0.3)  # Accelerating weakness
        
        return sum(gap_factors)
    
    def _calculate_price_descent_rate(self, c1: pd.Series, c2: pd.Series, c3: pd.Series) -> float:
        """Calculate the rate of price descent across the pattern"""
        total_decline = (c1['close'] - c3['close']) / c1['close']
        
        # Normalize descent rate (5% total decline = 1.0 score)
        descent_rate = min(total_decline / 0.05, 1.0) if total_decline > 0 else 0.0
        
        # Acceleration analysis
        decline1 = (c1['close'] - c2['close']) / c1['close']
        decline2 = (c2['close'] - c3['close']) / c2['close']
        
        if decline2 > decline1:  # Accelerating decline
            acceleration_bonus = min((decline2 - decline1) / 0.02, 0.3)
        else:
            acceleration_bonus = 0.0
        
        return descent_rate + acceleration_bonus
    
    def _assess_three_crows_trend_context(self, data: pd.DataFrame, candle_idx: int) -> float:
        """Assess trend context for three black crows patterns"""
        ...
        # Pattern should appear after uptrend for maximum impact
        price_change = (context_data['close'].iloc[-1] - context_data['close'].iloc[0]) / context_data['close'].iloc[0]
        if price_change > 0.05:  # Uptrend context
            trend_factor = min(price_change / 0.2, 1.0)
            context_factors.append(trend_factor * 0.4)
        else:
            context_factors.append(0.2)  # Reduced effectiveness in downtrend
        
        # High level context (near highs)
        recent_high = context_data['high'].max()
        current_close = context_data['close'].iloc[-1]
        high_proximity = current_close / recent_high
        
        if high_proximity > 0.95:
            context_factors.append(0.3)
        elif high_proximity > 0.9:
            context_factors.append(0.2)
        else:
            context_factors.append(0.1)
        
        # RSI context (should be elevated)
    ...
    def _analyze_momentum_acceleration(self, data: pd.DataFrame, candle_idx: int) -> float:
        ...
        # RSI decline acceleration
        rsi_values = momentum_data['rsi'].dropna()
        if len(rsi_values) >= 4:
            rsi_trend = np.polyfit(range(len(rsi_values)), rsi_values, 1)[0]
            if rsi_trend < -0.5:  # Declining RSI
                acceleration_factors.append(min(abs(rsi_trend) / 2.0, 1.0) * 0.3)
            else:
        ...
        if len(macd_hist) >= 4:
            if macd_hist.iloc[-1] < macd_hist.iloc[-2] < macd_hist.iloc[-3]:
                acceleration_factors.append(0.25)  # Accelerating bearish momentum
        ...
        if len(velocity_data) >= 3:
            ...
            if recent_velocity < -0.01:  # Strong negative velocity
                acceleration_factors.append(min(abs(recent_velocity) / 0.03, 1.0) * 0.25)
        
        # Stochastic analysis
        if not pd.isna(momentum_data.iloc[-1]['stoch_k']):
            stoch_k = momentum_data.iloc[-1]['stoch_k']
            if stoch_k < 30:  # Oversold momentum
                acceleration_factors.append((30 - stoch_k) / 30 * 0.2)
        
        return sum(acceleration_factors)
    
    def _calculate_three_crows_strength(self, candle_progression: float, body_consistency: float,
                                       shadow_analysis: float, trend_context_score: float,
                                       momentum_acceleration: float, gap_analysis: float) -> float:
        """Calculate overall three black crows pattern strength"""
        strength_components = [
    ...
    def _analyze_volume_progression(self, patterns: List[ThreeBlackCrowsPattern], 
                                  data: pd.DataFrame) -> List[ThreeBlackCrowsPattern]:
    ...
    def _calculate_volume_progression_score(self, data: pd.DataFrame, pattern_idx: int) -> Dict[str, float]:
        ...
        try:
            ...
            # High volume on the final candle (distribution)
    ...
    def _analyze_gap_characteristics(self, patterns: List[ThreeBlackCrowsPattern], 
                                   data: pd.DataFrame) -> List[ThreeBlackCrowsPattern]:
    ...
    def _calculate_detailed_gap_analysis(self, data: pd.DataFrame, pattern_idx: int) -> float:
        ...
        try:
            ...
            # Gap sizes
            gap1 = (c1['close'] - c2['open']) / c1['close'] if c1['close'] != 0 else 0
            gap2 = (c2['close'] - c3['open']) / c2['close'] if c2['close'] != 0 else 0
            
            # Optimal gap analysis
            for gap in [gap1, gap2]:
                if 0.002 <= gap <= 0.008:  # Ideal small gap down
                    gap_factors.append(0.3)
                elif 0 <= gap <= 0.002:  # Very small gap
                    gap_factors.append(0.2)
                elif 0.008 < gap <= 0.02:  # Moderate gap
                    gap_factors.append(0.15)
                elif gap < 0:  # Gap up (negative)
    ...
    def _analyze_exhaustion_signals(self, patterns: List[ThreeBlackCrowsPattern], 
                                  data: pd.DataFrame) -> List[ThreeBlackCrowsPattern]:
        """Analyze exhaustion signals for patterns"""
        for pattern in patterns:
            pattern_idx = data.index.get_loc(pattern.timestamp)
            exhaustion_score = self._calculate_exhaustion_signals_score(data, pattern_idx)
            pattern.exhaustion_signals = exhaustion_score
            pattern.pattern_purity = self._calculate_pattern_purity_score(data, pattern_idx, pattern)
        
        return patterns
    
    def _calculate_exhaustion_signals_score(self, data: pd.DataFrame, pattern_idx: int) -> float:
        """Calculate exhaustion signals score"""
        try:
            ...
            if not pd.isna(current['rsi']):
                if current['rsi'] < 25:
                    ...
                    exhaustion_factors.append(0.2)
                elif current['rsi'] > 70:
                    exhaustion_factors.append(0.1)  # Still overbought (good for pattern)
            
            # Williams %R oversold
            if not pd.isna(current['williams_r']):
                if current['williams_r'] < -85:
                    exhaustion_factors.append(0.25)
                elif current['williams_r'] < -75:
                    exhaustion_factors.append(0.15)
            
            # Bollinger Band position
            if not pd.isna(current['bb_position']):
                if current['bb_position'] < 0.1:  # Near lower band
                    exhaustion_factors.append(0.2)
                elif current['bb_position'] < 0.25:
                    exhaustion_factors.append(0.1)
            
            # Volume exhaustion (declining volume despite price decline)
            ...
            if len(context_data) >= 6:
                price_lows = context_data['low'].iloc[-6:]
                rsi_values = context_data['rsi'].iloc[-6:].dropna()
                
                if len(rsi_values) >= 4:
                    price_trend = np.polyfit(range(len(price_lows)), price_lows, 1)[0]
                    rsi_trend = np.polyfit(range(len(rsi_values)), rsi_values, 1)[0]
                    
                    # Price making lower lows but RSI not confirming
                    if price_trend < 0 and rsi_trend > 0:
                        exhaustion_factors.append(0.25)
            
            return sum(exhaustion_factors)
            
        except Exception:
            return 0.5
    
    def _calculate_pattern_purity_score(self, data: pd.DataFrame, pattern_idx: int, 
                                      pattern: ThreeBlackCrowsPattern) -> float:
    ...
    def _predict_reversal_potential(self, patterns: List[ThreeBlackCrowsPattern], 
                                  data: pd.DataFrame) -> List[ThreeBlackCrowsPattern]:
        """Predict reversal potential using ML"""
        if not patterns:
            return patterns
        ...
        try:
            ...
            if len(features) < 10:
                for pattern in patterns:
                    pattern.reversal_potential = self._heuristic_reversal_potential(pattern)
                return patterns
            ...
            if self.is_ml_fitted:
                ...
                for i, pattern in enumerate(patterns):
                    ml_potential = max(0.1, min(0.95, reversal_predictions[i]))
                    heuristic_potential = self._heuristic_reversal_potential(pattern)
                    pattern.reversal_potential = (ml_potential * 0.7 + heuristic_potential * 0.3)
            else:
                for pattern in patterns:
                    pattern.reversal_potential = self._heuristic_reversal_potential(pattern)
    ...
    def _extract_reversal_features(self, data: pd.DataFrame, pattern_idx: int, 
                                 pattern: ThreeBlackCrowsPattern) -> List[float]:
        ...
        try:
            ...
            features = [
                ...
                pattern.price_descent_rate,
                pattern.volume_confirmation,
                pattern.exhaustion_signals,
                ...
                current['body_ratio'],
                pattern.momentum_acceleration * pattern.exhaustion_signals,  # Combined signal
    ...
    def _train_reversal_model(self, patterns: List[ThreeBlackCrowsPattern], features: List[List[float]]):
        """Train ML model for reversal prediction"""
        try:
            targets = []
            for pattern in patterns:
                # Target combines multiple factors for reversal potential
                target = (
                    pattern.pattern_strength * 0.3 +
                    pattern.exhaustion_signals * 0.25 +
    ...
    def _heuristic_reversal_potential(self, pattern: ThreeBlackCrowsPattern) -> float:
        """Calculate heuristic reversal potential"""
        return (
            pattern.pattern_strength * 0.3 +
            pattern.exhaustion_signals * 0.25 +
            pattern.volume_confirmation * 0.2 +
    ...
    def _generate_pattern_analytics(self, patterns: List[ThreeBlackCrowsPattern]) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_reversal_potential': sum(p.reversal_potential for p in recent_patterns) / len(recent_patterns),
            'average_descent_rate': sum(p.price_descent_rate for p in recent_patterns) / len(recent_patterns),
            'high_strength_patterns': len([p for p in recent_patterns if p.pattern_strength > 0.8]),
            'high_potential_patterns': len([p for p in recent_patterns if p.reversal_potential > 0.75]),
            'strong_exhaustion_patterns': len([p for p in recent_patterns if p.exhaustion_signals > 0.7]),
    ...
    def _analyze_current_trend_context(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'bearish_momentum': current['momentum_roc'] < -0.02 if not pd.isna(current['momentum_roc']) else False
        }
    
    def _generate_reversal_signals(self, patterns: List[ThreeBlackCrowsPattern], 
        ...
        return {
            ...
            'reversal_probability': sum(p.reversal_potential for p in recent_patterns) / len(recent_patterns),
            'exhaustion_level': sum(p.exhaustion_signals for p in recent_patterns) / len(recent_patterns),
            'volume_confirmation': sum(p.volume_confirmation for p in recent_patterns) / len(recent_patterns),
            'pattern_count': len(recent_patterns),
            'descent_rate': sum(p.price_descent_rate for p in recent_patterns) / len(recent_patterns),
    ...
    def _analyze_market_structure(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            'bearish_candle_frequency': len([i for i in range(len(recent_data)) if recent_data.iloc[i]['is_bearish']]) / len(recent_data),
    ...
    def _assess_pattern_reliability(self, patterns: List[ThreeBlackCrowsPattern]) -> Dict[str, Any]:
        ...
        return {
            ...
            'exhaustion_reliability': np.mean([p.exhaustion_signals for p in patterns]),
            'volume_reliability': np.mean([p.volume_confirmation for p in patterns]),
            'reversal_success_estimate': np.mean([p.reversal_potential for p in patterns])
        }
    
    def _calculate_descent_statistics(self, patterns: List[ThreeBlackCrowsPattern]) -> Dict[str, Any]:
        """Calculate descent-specific statistics"""
        if not patterns:
            return {}
        
        descent_rates = [p.price_descent_rate for p in patterns]
        momentum_accelerations = [p.momentum_acceleration for p in patterns]
        
        return {
            'descent_rate_stats': {
                'mean': np.mean(descent_rates),
                'median': np.median(descent_rates),
                'max': np.max(descent_rates),
                'std': np.std(descent_rates)
            },
            'momentum_stats': {
    ...
    def _generate_signal(self, value: Dict[str, Any], data: pd.DataFrame) -> Tuple[Optional[SignalType], float]:
        """Generate trading signal based on three black crows analysis"""
        ...
        if (current_pattern.pattern_strength > 0.8 and
            current_pattern.reversal_potential > 0.75 and
            current_pattern.exhaustion_signals > 0.6):
            
            confidence = (
                current_pattern.pattern_strength * 0.35 +
                current_pattern.reversal_potential * 0.3 +
                current_pattern.exhaustion_signals * 0.2 +
        ...
        # Moderate bearish signal
              ...
              current_pattern.reversal_potential > 0.65):
    ...
    def _get_metadata(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        base_metadata.update({
            ...
            'max_upper_shadow_ratio': self.parameters['max_upper_shadow_ratio'],
