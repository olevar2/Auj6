"""
Robust Hourly Feedback Loop Manager for AUJ Platform.

This module implements the critical hourly feedback loop that integrates all anti-overfitting
...
class FeedbackLoopPhase(str, Enum):
    """Phases of the hourly feedback loop."""
...
class RobustHourlyFeedbackLoop:
    """
    Robust Hourly Feedback Loop Manager.
    
    This component orchestrates the hourly optimization and validation cycle that ensures
    ...
    - Coordinate hourly optimization cycles
    ...
    def __init__(self,
        ...
        Args:
            ...
            economic_monitor: Economic calendar monitoring (optional)
        ...
        self.logger.info("RobustHourlyFeedbackLoop initialized successfully")
    ...
    def _initialize_database(self):
        ...
        try:
            ...
            """, use_cache=False)
            self.logger.info("Feedback loop database initialized")
    ...
    async def start_feedback_loop(self):
        """Start the hourly feedback loop."""
        try:
            self.logger.info("Starting Robust Hourly Feedback Loop")
            ...
            while not self.emergency_stop_triggered:
                ...
                if self.next_execution is None or current_time >= self.next_execution:
                    ...
                    self.next_execution = current_time + self.loop_frequency
                
                # Wait before next check (1 minute intervals)
                await asyncio.sleep(60)
                
        except Exception as e:
            self.logger.error(f"Feedback loop error: {str(e)}")
    ...
    async def execute_feedback_cycle(self) -> FeedbackLoopMetrics:
        ...
        try:
            self.logger.info(f"Starting feedback cycle: {loop_id}")
            ...
            # Phase 10: Execute Genius Agent Coordinator Analysis Cycle
            phase_start = datetime.utcnow()
            ...
            self.logger.info(
                f"Feedback cycle {loop_id} completed successfully in {duration:.2f}s. "
            ...
            self.logger.error(f"Feedback cycle {loop_id} failed: {str(e)}")
    ...
    async def _save_performance_trend(self, 
                                    performance: float, 
                                    overfitting_risk: float, 
                                    stability_score: float):
        """Save performance trend data with comprehensive analysis metrics."""
        try:
            current_regime = await self.regime_classifier.classify_current_regime()
            ...
            self.database.execute_query_sync("""
                INSERT INTO performance_trends (
                    timestamp, overall_performance, overfitting_risk, stability_score,
                    active_agents, elite_indicators_count, market_regime,
                    comprehensive_analysis_rate, analysis_consistency_score, full_validation_rate
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                datetime.utcnow().isoformat(),
                performance,
                overfitting_risk,
                stability_score,
                len(self.agent_optimizer.agent_profiles),
                len(self.indicator_analyzer.get_elite_indicators(current_regime)),
                current_regime.value,
    ...
    def _create_execution_metrics(self,
                                loop_id: str,
                                start_time: datetime,
                                duration: float,
                                phase_durations: Dict[str, float],
                                ...
                                hierarchy_changes: Dict[str, str],
                                indicator_updates: Dict[str, Any],
                                elite_indicator_changes: Dict[str, Any],
                                validation_result: Dict[str, Any],
                                system_status: Dict[str, Any]) -> FeedbackLoopMetrics:
        ...
        elite_indicators_updated = len(elite_indicator_changes.get('added_indicators', [])) + len(elite_indicator_changes.get('removed_indicators', []))
        indicators_promoted = len(indicator_updates.get('promoted_indicators', []))
        indicators_demoted = len(indicator_updates.get('demoted_indicators', []))
        ...
        if system_status.get('overfitting_risk', 0.0) > 0.4:
            recommendations.append('INCREASE_REGULARIZATION_STRENGTH')
        
        if system_status.get('stability_score', 1.0) < 0.6:
            recommendations.append('REDUCE_LEARNING_RATES')
        
        if agents_requiring_intervention > len(self.agent_optimizer.agent_profiles) * 0.5:
            recommendations.append('REVIEW_AGENT_CONFIGURATIONS')
        
        if not recommendations:
            recommendations.append('SYSTEM_OPERATING_OPTIMALLY')
    ...
    def get_feedback_loop_status(self) -> Dict[str, Any]:
        """Get current feedback loop status and metrics."""
        return {
            ...
            'system_changes_this_cycle': self.system_changes_count,
            'last_successful_state': {
                'timestamp': self.last_successful_state['timestamp'].isoformat(),
                'performance': self.last_successful_state['performance_metrics']
            } if self.last_successful_state else None
        }
    
    def get_execution_history(self, limit: int = 20) -> List[Dict[str, Any]]:
        """Get recent execution history."""
        recent_metrics = self.execution_metrics[-limit:] if self.execution_metrics else []
        ...
        return [
            {
                ...
                'overall_performance': m.overall_system_performance,
                'overfitting_risk': m.overfitting_risk_score,
                'stability_score': m.stability_score,
                'agents_optimized': m.agents_optimized,
                'validation_success': m.validation_success,
                'recommendations': m.system_recommendations,
                'risk_warnings': m.risk_warnings
            }
            for m in recent_metrics
        ]
    
    async def manual_trigger_feedback_cycle(self) -> FeedbackLoopMetrics:
        """Manually trigger a feedback cycle (for testing or emergency)."""
        self.logger.info("Manually triggering feedback cycle")
        
        # Save current next_execution time
        original_next_execution = self.next_execution
        
        try:
            # Execute the feedback cycle
            metrics = await self.execute_feedback_cycle()
            
            # Restore original schedule
            self.next_execution = original_next_execution
            
            return metrics
            
        except Exception as e:
            # Restore original schedule even on error
            self.next_execution = original_next_execution
            raise
    
    async def reset_emergency_stop(self, reason: str = "Manual reset"):
        """Reset emergency stop condition."""
        if self.emergency_stop_triggered:
            self.logger.info(f"Resetting emergency stop: {reason}")
            
            self.emergency_stop_triggered = False
            self.current_status = FeedbackLoopStatus.IDLE
            
            # Update emergency intervention record
            try:
                # Using unified database manager instead of direct sqlite3
                self.database.execute_query_sync("""
                    UPDATE emergency_interventions 
                    SET resolution_status = ?, resolution_timestamp = ?
                ...
                self.logger.warning(f"Failed to update emergency intervention record: {str(e)}")
    
    async def _execute_economic_monitoring(self):
        """Execute economic monitoring tasks."""
        try:
            if self.economic_monitor:
                ...
                await self.economic_monitor.refresh_economic_data()
                
                self.logger.info("Economic monitoring cycle completed")
            else:
                self.logger.debug("Economic monitor not available - skipping monitoring")
                
        except Exception as e:
            self.logger.error(f"Error in economic monitoring execution: {str(e)}")
    
    async def shutdown(self):
        """Gracefully shutdown the feedback loop."""
        self.logger.info("Shutting down Robust Hourly Feedback Loop")
